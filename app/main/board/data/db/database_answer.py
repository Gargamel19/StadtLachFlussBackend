...
class DatabaseAnswer:
    ...
    def get_answers_from_category_by_category_id(category_id):
        ...
        for cat in answer:
            if cat:
                if bool(cat[4]):
                    answers.append(Answer(int(cat[0]), int(cat[1]), int(cat[2]), cat[3], "True"))
                else:
                    answers.append(Answer(int(cat[0]), int(cat[1]), int(cat[2]), cat[3], "False"))
        return answers
    ...
    def get_by_answer_id(answer_id):
        ...
        if answer and len(answer) == 1:
            player_obj = answer[0]
            if player_obj:
                if bool(player_obj[4]):
                    answer = Answer(int(player_obj[0]), int(player_obj[1]), int(player_obj[2]), player_obj[3], "True")
                else:
                    answer = Answer(int(player_obj[0]), int(player_obj[1]), int(player_obj[2]), player_obj[3], "False")
                return answer
            else:
                return player_obj
        else:
    ...
    def update_answer_visibility(answer_id, visibility):
        if visibility:
            query = "UPDATE ANSWER SET ANSWER_VISIBLE = 'True' WHERE ANSWER_ID = {}".format(answer_id)
        else:
            query = "UPDATE ANSWER SET ANSWER_VISIBLE = 'False' WHERE ANSWER_ID = {}".format(answer_id)
